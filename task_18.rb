class ResumeBuilder
	...
	def self.build
		...
		@save_user_info = Media.new(user_info)
		@save_user_info.show_choices()
		@save_user_info.save_file_as()
	end
...
class Media
	...
	def show_choices()
		puts <<-CHOICES
		Which type of file u want to convert into?
		1 .txt file
		2 .csv file
		3 .pdf file
		CHOICES
		...
		@save_text = TextMedia.new(resume)
		@save_csv = CsvMedia.new(resume)
		@save_pdf = PdfMedia.new(resume)
		if choice == "1"
			@save_text.save_file_as()
		elsif choice == "2"
			@save_csv.save_file_as()
		elsif choice == "3"
			@save_pdf.save_file_as()
		else
			puts"Cannot proceed!!!"
		end
	end
